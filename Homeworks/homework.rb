...
users.map! do |user|
  ...
  unique_hobbies = hobbies - user[:hobbies]
  unique_hobbies.delete(:football)
  if user[:hobbies].count < 2 && user[:hobbies].include?(:football)  
    user[:hobbies] << unique_hobbies.sample(1).first
  elsif user[:hobbies].count < 2 && !user[:hobbies].include?(:football)
